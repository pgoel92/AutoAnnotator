...
def align(ExpectedTokens,DecodedTokens,DecodedTime):

	r = len(ExpectedTokens)+1;	#No. of rows in my DP matrix
	c = len(DecodedTokens)+1;	#No. of columns in my DP matrix
	...
	for i in range(1,r):
		for j in range(1,c):
			if ExpectedTokens[i-1] == DecodedTokens[j-1]:
				DPMatrix[i][j] = DPMatrix[i-1][j-1]
				PathMatrix[i][j] = 0
			else:
				mymin=minimum(DPMatrix[i-1][j],DPMatrix[i][j-1],DPMatrix[i-1][j-1]);
	...
	ExpectedOutput = []		#Formatted expected output after alignment
	DecodedOutput = []		#Formatted decoded output after alignment
	...
	while i>0 or j>0:
		...
		if PathMatrix[i][j] == 0:			#If correct match
			...
			ExpectedOutput.insert(0,ExpectedTokens[i-1]);
			DecodedOutput.insert(0,DecodedTokens[j-1]);
			matches = matches + 1
			i = i-1
			j = j-1
			...
			out = formatting(ExpectedTokens[i-1],DecodedTokens[j-1]);
			ExpectedOutput.insert(0,out[0]);
			DecodedOutput.insert(0,out[1]);
			
			substitutions = substitutions + 1
			i = i-1
			j = j-1
		
		elif PathMatrix[i][j] == 2:			#If insertion
			
			out = formatting(ExpectedTokens[i-1],"***");
			ExpectedOutput.insert(0,out[0]);
			DecodedOutput.insert(0,out[1]);
			...
			out = formatting("***",DecodedTokens[j-1]);
			ExpectedOutput.insert(0,out[0]);
			DecodedOutput.insert(0,out[1]);
	...
	isl_begin = []
	isl_end = []
	ExpectedTime = [None for _ in range(r)]
	...
	while i>0 and j>0:	
		...
		if PathMatrix[i][j] == 0:
			...
			IslandLength = IslandLength + 1	#Inc no. of matches in current island by 1

		elif PathMatrix[i][j] == 1:		#If substitution
			if isl_flag == 1 and IslandLength >= ConfidenceThreshold :	#If island of confidence
				startexp = i
				startdec = j
				for k in range(0,enddec-startdec):
					ExpectedTime[startexp+k]=DecodedTime[startdec+k]		
				isl_begin.insert(0,startexp);
				isl_end.insert(0,endexp);
			IslandLength = 0
			isl_flag = 0
		elif PathMatrix[i][j] == 2:		#If insertion	
			if IslandLength >= ConfidenceThreshold and isl_flag == 1:				
				startexp = i
				startdec = j
				for k in range(0,enddec-startdec):
					ExpectedTime[startexp+k]=DecodedTime[startdec+k]		
	...
	for i in range(0,r-1):
		f.write(ExpectedTokens[i] + " ");
		if ExpectedTime[i]!=None:
			for elt in ExpectedTime[i]:
				f.write(str(elt)+" ");
		 	f.write("\n");	
		else: f.write("None\n");
	f.close();
	...
	while k < r-1:
		if ExpectedTime[k] is None:
			if k>0: 
				audio_begin = ExpectedTime[k-1][1] + 1 
				isl_exp_ind = k
			while k < r-1 and ExpectedTime[k] == None:
				text = text + ExpectedTokens[k] + " "
				k = k+1
			audio_end = ExpectedTime[k][0]
...
try:
	...
	for elt in inp:
		...
		DecodedTime.append(temp);
	
	align(expected,decoded,DecodedTime);
